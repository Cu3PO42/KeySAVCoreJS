...
export async function load(input: Uint8Array): Promise<SaveReader> {
  ...
  switch (input.length) {
    ...
    case 0xfe000:
      ...
      return new SaveReaderEncrypted(input, key);
    case 0x76000:
      if (view.getUint32(0x75e10, true) != magic) throw createNotASaveError();
      return new SaveReaderDecrypted(input, "ORAS");
    case 0x65600:
      if (view.getUint32(0x65410, true) != magic) throw createNotASaveError();
...
function upgradeKey(key: SaveKey, break1: Uint8Array, break2: Uint8Array): { result: number; pkx?: PkBase } {
  ...
  util.xor(
    ...
    key.boxOffset,
    break1,
    key.boxOffset - offsets.saveSize,
    key.slot1Key,
    0,
    232 * 30 * (generation === 6 ? 31 : 32)
  );
...
export async function breakKey(break1: Uint8Array, break2: Uint8Array): Promise<string> {
  ...
  try {
    ...
    if (e.name === "SaveKeyAlreadyExistsError") throw e;
  }
  ...
  var potentialOffsets =
    generation1 === 6 ? [0x26a00 /* XY */, 0x37400 /* ORAS */] : [0x8200 /* SM */, 0x8600 /* USUM */];
  ...
  if (result.result === 2) {
    ...
    for (let i of indices) {
      for (let empty of [ezeros, emptyEkx]) {
        if (
          PkBase.verifyChk(PkBase.decrypt(util.xorThree(boxes1, i + 232 * 30, empty, 0, boxes2, i + 232 * 30, 232)))
        ) {
