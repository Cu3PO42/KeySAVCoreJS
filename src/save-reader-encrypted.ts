...
export default class SaveReaderEncrypted implements SaveReader {
  ...
  get unlockedSlots() {
    var res = 0;
    for (var i = 0; i < this.key.slotsUnlocked.length; ++i) if (this.key.slotsUnlocked[i]) ++res;
    return res;
  }
  ...
  constructor(private sav: Uint8Array, private key: SaveKey) {
    ...
    this.activeSlot =
      this.key.slot1Flag == util.createDataView(sav).getUint32(0x168, true) && this.key.isNewKey ? 0 : 1;
    this.boxes1 = util.xor(
      this.sav,
      key.boxOffset - offsets.saveSize,
      key.slot1Key,
      0,
      232 * 30 * (this.generation === 6 ? 31 : 32)
    );
  ...
  getPkx(pos: number): PkBase {
    var res = SaveReaderEncrypted.getPkxRaw(this.activeSlot === 0 ? this.boxes1 : this.boxes2, pos, this.key),
      data = res[0],
      ghost = res[1];
    if (data === undefined || (data[8] | data[9]) == 0) return undefined;
