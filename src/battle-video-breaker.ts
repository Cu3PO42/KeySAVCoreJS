...
function breakParty(
  reader1: BattleVideoReader,
  reader2: BattleVideoReader,
  partyOffset: number,
  key: BattleVideoKey,
  partyNumber: number
): boolean {
  ...
  if (
    reader1.getPkx(0, partyNumber) === undefined ||
    reader1.getPkx(1, partyNumber) !== undefined ||
    reader2.getPkx(1, partyNumber) === undefined ||
    reader2.getPkx(2, partyNumber) !== undefined
  ) {
 ...
 */
export async function breakKey(
  video1: Uint8Array,
  video2: Uint8Array
): Promise<{ upgraded: boolean; workingKeys: boolean[] }> {
