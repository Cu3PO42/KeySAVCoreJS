...
namespace KeySAVCore
{
    public static class SaveBreaker
    {
        ...
        public static void Break(Uint8Array break1, Uint8Array break2, JsFunction keyGetter, JsFunction callback)
        {
            ...
            {
                callback.invoke(new SaveBreakResult(false, "Saves are not from the same game!\nPlease follow the instructions.", null, null));
                return;
            }

            if(Utility.SequenceEqual(break1, break2))
            {
                callback.invoke(new SaveBreakResult(false, "The saves are identical.\nPlease follow the instructions.", null, null));
                return;
            }

            keyGetter.invoke(BitConverter.ToUInt32(break1, 0x10), BitConverter.ToUInt32(break1, 0x14), (Action<Exception, SaveKey>)((e, key) =>
            {
                if (e == null)
                {
                    ...
                    callback.invoke(new SaveBreakResult(true, "Found old key. Based new keystream on that.\n\nPlease save new Keystream.", key, null));
                    return;
                }
                if (Utility.SequenceEqual(break1, 0x80000, break2, 0x80000, 0x7F000))
                {
                ...
                if ((offset[0] == 0) || (offset[1] == 0))
                {
                    ...
                    callback.invoke(new SaveBreakResult(false, result, null, null));
                    return;
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                // Sigh...
                                callback.invoke(new SaveBreakResult(false, "", null, null));
                                return;
                            }
                        }
                        #endregion
                ...
                if (true)
                {
                    ...
                    callback.invoke(new SaveBreakResult(true, result, tmp, pkx));
                    return;
                }
            }));
        }
